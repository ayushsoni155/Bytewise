...
export default async function handler(req, res) {
  ...
  try {
    ...
    // For each order, fetch the associated order items and product names
    const ordersWithItems = await Promise.all(orders.map(async (order) => {
      const [orderItems] = await conn.query(
        `
        SELECT oi.*, pb.product_name 
        FROM order_items oi
      ...
      return {
        ...order,
        items: orderItems, // Attach order items with product names to the order
      };
    }));
